"""Budgeted ultra-long simulation tests (fast, no huge RAM)."""
...
def test_scale_demo_simulate_fast(tmp_path: Path) -> None:
    result = run_scale_demo(
        target_in=10_000_000,
        target_out=1_000_000,
        simulate=True,
        out_dir=tmp_path / "demo",
        max_materialized_pages=40,
    )
    assert result.input_tokens_est == 10_000_000
    ...
    assert len(p0) > 0
    # Honesty: prompt-only extreme ratios may or may not hold; factor uses simulated target
...
def test_prompt_pack_under_budget_on_repetitive(tmp_path: Path) -> None:
    ...
    comp = BudgetedContextCompressor(output_token_budget=50_000, hot_page_fraction=0.05)
